...
wireOne.forEach(function (instruction) {
...
wireTwo.forEach(function (instruction) {
    var direction = instruction.substring(0, 1);
    var distance = parseInt(instruction.substring(1));
    for (distance; distance > 0; distance--) {
        var newPos = move(xPos, yPos, direction);
...
var intersectingCoords = wireOneCoords.filter(function (x) { return wireTwoCoords.includes(x); });
console.log("Finding Manhattan distances for " + intersectingCoords.length + " intersections.");
intersectingCoords.forEach(function (i) {
...
var shortestDistance = Math.min.apply(Math, manDistances);
